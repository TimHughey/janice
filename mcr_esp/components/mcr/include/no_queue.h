 ...
 * hold the items in the queue. &xQueueBuffer ); // The buffer that will hold
 * the queue structure.
 *
 ...
 * of the call.	 The time is defined in tick periods so the constant
 * portTICK_PERIOD_MS should be used to convert to real time if this is
 * required. xQueueGenericReceive() will return immediately if the queue is
 * empty and xTicksToWait is 0.
 *
 ...
 * task. portYIELD_FROM_ISR(); // or portEND_SWITCHING_ISR() depending on the
 * port.
 * 	}
 * }
 * @endcode
 ...
 * be copied.
 *
 * @param[out] pxHigherPriorityTaskWoken A task may be blocked waiting for space
 * to become available on the queue.  If xQueueReceiveFromISR causes such a task
 ...
 * This is	the approach taken by many other RTOSes, but FreeRTOS.org has
 * the preferred fully featured API too.  The fully featured API has more
...
void vQueueWaitForMessageRestricted(
    QueueHandle_t xQueue, TickType_t xTicksToWait) PRIVILEGED_FUNCTION;
