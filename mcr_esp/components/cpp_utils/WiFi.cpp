 ...
 */
WiFi::~WiFi() { delete m_pWifiEventHandler; }

/**
 * @brief Add a reference to a DNS server.
 *
 * Here we define a server that will act as a DNS server.  We can add two DNS
 * servers in total.  The first will be the primary, the second will be the
 * backup. The public Google DNS servers are "8.8.8.8" and "8.8.4.4".
 *
 * For example:
 *
 * @code{.cpp}
...
void WiFi::addDNSServer(ip_addr_t ip) {
  ESP_LOGD(LOG_TAG, "Setting DNS[%d] to %d.%d.%d.%d", m_dnsCount,
 ...
 *
 * Here we define a server that will act as a DNS server.  We use numdns to
 * specify which DNS server to set
 *
 * For example:
 *
 * @code{.cpp}
 ...
 *
 * @param [in] ssid The network SSID of the access point to which we wish to
 * connect.
 * @param [in] password The password of the access point to which we wish to
 * connect.
 * @param [in] waitForConnection Block until the connection has an outcome.
 * @return N/A.
 */
bool WiFi::connectAP(const std::string &ssid, const std::string &password,
  ...
  m_connectFinished.take(
      "connectAP"); // Take the semaphore to wait for a connection.
  ...
  m_connectFinished.wait(
      "connectAP"); // Wait for the completion of the connection.
  ESP_LOGD(LOG_TAG, "<< connectAP");
...
/* STATIC */ esp_err_t WiFi::eventHandler(void *ctx, system_event_t *event) {
  // This is the common event handler that we have provided for all event
  // processing.  It is called for every event that is received by the WiFi
  // subsystem.  The "ctx" parameter is an instance of the current WiFi object
  // that we are processing.  We can then retrieve the specific/custom event
  // handler from within it and invoke that.  This then makes this an
  // indirection vector to the real caller.

  WiFi *pWiFi =
      (WiFi *)ctx; // retrieve the WiFi object from the passed in context.

  // Invoke the event handler.
  esp_err_t rc = pWiFi->m_pWifiEventHandler->getEventHandler()(
      pWiFi->m_pWifiEventHandler, event);

  // If the event we received indicates that we now have an IP address or that a
  // connection was disconnected then unlock the mutex that indicates we are
  // waiting for a connection complete.
  if (event->event_id == SYSTEM_EVENT_STA_GOT_IP ||
      event->event_id == SYSTEM_EVENT_STA_DISCONNECTED) {
    if (event->event_id ==
        SYSTEM_EVENT_STA_GOT_IP) { // If we connected and have an IP, change the
                                   // flag.
...
/* PRIVATE */ void WiFi::init() {
  ...
  // If we have already started the event loop, then change the handler
  // otherwise start the event loop.
  if (m_eventLoopStarted) {
    esp_event_loop_set_cb(WiFi::eventHandler, this); // Returns the old handler.
  } else {
    esp_err_t errRc = ::esp_event_loop_init(
        WiFi::eventHandler, this); // Initialze the event handler.
    if (errRc != ESP_OK) {
...
std::vector<WiFiAPRecord> WiFi::scan() {
  ...
  std::sort(apRecords.begin(), apRecords.end(),
            [](const WiFiAPRecord &lhs, const WiFiAPRecord &rhs) {
              return lhs.m_rssi > rhs.m_rssi;
            });
  return apRecords;
} // scan
 ...
 * @param[in] auth The authorization mode for access to this access point.
 * Options are:
 * * WIFI_AUTH_OPEN
 * * WIFI_AUTH_WPA_PSK
 * * WIFI_AUTH_WPA2_PSK
...
/**
 * @brief Set the IP info and enable DHCP if ip != 0. If called with ip == 0
 * then DHCP is enabled. If called with bad values it will do nothing.
 *
 ...
 * @return N/A.
 */
void WiFi::setIPInfo(const std::string &ip, const std::string &gw,
...
std::string WiFiAPRecord::toString() {
  ...
  sprintf(info_str, "ssid: %s, auth: %s, rssi: %d", m_ssid.c_str(),
          auth.c_str(), (int)m_rssi);
...
MDNS::MDNS() {
        ...
        if (errRc != ESP_OK) {
                ESP_LOGE(LOG_TAG, "mdns_init: rc=%d %s", errRc,
 ...
 */
/*
void MDNS::serviceAdd(const std::string& service, const std::string& proto,
uint16_t port) { serviceAdd(service.c_str(), proto.c_str(), port); } //
serviceAdd


void MDNS::serviceInstanceSet(const std::string& service, const std::string&
proto, const std::string& instance) { serviceInstanceSet(service.c_str(),
proto.c_str(), instance.c_str()); } // serviceInstanceSet


void MDNS::servicePortSet(const std::string& service, const std::string& proto,
uint16_t port) { servicePortSet(service.c_str(), proto.c_str(), port); } //
servicePortSet
...
void MDNS::serviceAdd(const char* service, const char* proto, uint16_t port) {
        esp_err_t errRc = ::mdns_service_add(m_mdns_server, service, proto,
port); if (errRc != ESP_OK) { ESP_LOGE(LOG_TAG, "mdns_service_add: rc=%d %s",
errRc, GeneralUtils::errorToString(errRc)); abort();
        }
} // serviceAdd


void MDNS::serviceInstanceSet(const char* service, const char* proto, const
char* instance) { esp_err_t errRc = ::mdns_service_instance_set(m_mdns_server,
service, proto, instance); if (errRc != ESP_OK) { ESP_LOGE(LOG_TAG,
"mdns_service_instance_set: rc=%d %s", errRc,
GeneralUtils::errorToString(errRc)); abort();
  }
} // serviceInstanceSet
...
{ esp_err_t errRc = ::mdns_service_port_set(m_mdns_server, service, proto,
port); if (errRc != ESP_OK) { ESP_LOGE(LOG_TAG, "mdns_service_port_set: rc=%d
%s", errRc, GeneralUtils::errorToString(errRc)); abort();
  }
} // servicePortSet
...
void MDNS::setHostname(const char* hostname) {
  esp_err_t errRc = ::mdns_set_hostname(m_mdns_server,hostname);
    if (errRc != ESP_OK) {
        ESP_LOGE(LOG_TAG, "mdns_set_hostname: rc=%d %s", errRc,
GeneralUtils::errorToString(errRc)); abort();
    }
} // setHostname
*/
...
void MDNS::setInstance(const char* instance) {
    ...
    if (errRc != ESP_OK) {
        ESP_LOGE(LOG_TAG, "mdns_set_instance: rc=%d %s", errRc,
GeneralUtils::errorToString(errRc)); abort();
    }
} // setInstance
*/
