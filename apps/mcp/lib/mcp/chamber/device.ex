defmodule Mcp.Chamber.Device do
  def license, do: """
    ...
    defstruct name: @def_name, ts: Timex.zero(), ctrl_dev: "foobar",
      status: @disabled, pending_timer: :nil
    ...
    when (status == @idling or status == @disabled) and
    (is_reference(timer) or timer == :nil) do
    ...
    when is_reference(timer) or timer == :nil, do: dev
    ...
    when (status == @running or status == @disabled) and
      (is_reference(timer) or timer == :nil) do
      # if there was an existing pending timer cancel it as the new
      # timer will override
      cancel_timer(dev)
