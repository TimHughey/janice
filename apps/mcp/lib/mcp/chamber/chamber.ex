defmodule Mcp.Chamber do
  ...
  defp need_warm?(%Chamber{warm: true} = c) do
    pri = Sensor.value(c.temp_sensor_pri, "temperature")
    sec = Sensor.value(c.temp_sensor_sec, "temperature")
    need_warm?(c, pri, sec)
  end
  ...
  defp need_mist?(%Chamber{} = c, %RunState{}, :relhum) do
    rh = Sensor.value(c.relh_sensor, "humidity")
    max = c.relh_setpt
    min = c.relh_setpt
  ...
  defp schedule_routine_check do
    send_after(self(), @routine_check_msg, config(:routine_check_ms))
  end
  ...
  def start_link(_args) do
    start_link(Mcp.Chamber, config(:name), %RunState{})
  end
  ...
  def init(_opts) do
    ...
    AutoPopulate.populate(config(:auto_populate))

    send_after(self(), @start_msg, config(:kickstart_wait_ms))
  ...
  def disabled_list do
    GenServer.call(server_name(), @disabled_list_msg)
  end

  @known_msg {:known_list_msg}
  def known, do: GenServer.call(server_name(), @known_msg)
  ...
  def status(name) when is_binary(name) do
    GenServer.call(server_name(), {@chamber_status_msg, name})
  end
  #
  # GenServer callbacks
  #
  ...
  when is_binary(n) do
    ...
    msg = {@fresh_air_msg, n}
    send_after(self(), msg, config(:routine_check_ms))
