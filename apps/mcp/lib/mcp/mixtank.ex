...
#    along with this program.  If not, see <http://www.gnu.org/licenses/>
  ...
  def start_link(_args) do
    start_link(Mcp.Mixtank, config(:name), %State{})
  end
  ...
  defp ensure_inactive(%State{} = state, %Mixtank{} = mixtank) do
    ...
    if not State.tank_disabled?(state, name) do
      ...
      Switch.change(mixtank.heat_sw, :false)
      Switch.change(mixtank.air_sw, :false)
      Switch.change(mixtank.pump_sw, :false)
  ...
  defp follow_ref(%State{} = state, %Mixtank{} = mixtank) do
    ...
    Switch.change(mixtank.heat_sw, next_position)
    ...
    # always post the transition
    # commented out until this can be reworked to handle multiple mixtanks
    # (or profiles) handling the same switches (transitions)
    # mixtank.name |> Transition.create("heater", next_position) |> Transition.post()

    State.set_heat_status(state, name, next_position)
  end

  defp sensor_value(s) do
    case Sensor.get_by(s, "temperature") do
      {:ok, {_, _, val, _}} -> val
      {:stale, {_, _, _, _}} -> :nil
    end
  end
  ...
  when is_boolean(power) do
    ...
    Switch.change(mixtank.pump_sw, power)
  ...
  when is_boolean(power) do
    ...
    Switch.change(mixtank.air_sw, power)
  ...
  def manual_control(code) when is_integer(code) do
    GenServer.cast(server_name(), {@manual_control_msg, code})
  end

  @known_tanks_msg :known_mixtanks

  def known_tanks do
    GenServer.call(server_name(), @known_tanks_msg)
  end
  ...
  defp control_temp_ms, do: config(@control_temp_ms)
  defp activate_ms, do: config(@activate_ms)
  defp manage_ms, do: config(@manage_ms)
end
