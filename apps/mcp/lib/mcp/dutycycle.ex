...
#    along with this program.  If not, see <http://www.gnu.org/licenses/>
  ...
  def start_link(_args) do
    start_link(Mcp.Dutycycle, config(:name), %State{})
  end
  ...
  def init(state) do
    ...
    auto_populate()
    Process.send_after(self(), {:start}, config(:kickstart_wait_ms))
  ...
  def disabled_cycles do
    GenServer.call(server_name(), {:get_disabled_cycles})
  end

  def known_cycles do
    GenServer.call(server_name(), @known_dutycycles_msg)
  end
...
#       device_sw: "proxr_test", run_ms: 3*60*1000, idle_ms: 2*60*1000}
  ...
  when is_boolean(power) do
    Switch.change(dutycycle.device_sw, power)
  ...
  defp schedule_routine_check do
    Process.send_after(self(), {:routine_check}, config(:routine_check_ms))
  end
end
