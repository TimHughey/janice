defmodule Mcp.Chamber.RunState do
  ...
  def heater(%RunState{} = rs, pos, t)
    ...
    %RunState{rs | @heater => Device.position(rs.heater, pos, t), @ts => Timex.now()}
  end
  ...
  def air_stir(%RunState{} = rs, pos, t)
      when is_boolean(pos) and (is_reference(t) or t == nil) do
    %RunState{rs | @air_stir => Device.position(rs.air_stir, pos, t), @ts => Timex.now()}
  end
  ...
  def mist(%RunState{} = rs, pos, t)
      when is_boolean(pos) and (is_reference(t) or t == nil) do
    %RunState{rs | @mist => Device.position(rs.mist, pos, t), @ts => Timex.now()}
  end
  ...
  def fresh_air(%RunState{} = rs, pos, t)
      when is_boolean(pos) and (is_reference(t) or t == nil) do
    %RunState{rs | @fresh_air => Device.position(rs.fresh_air, pos, t), @ts => Timex.now()}
  end
  ...
  def disable_all(%RunState{} = rs) do
    %RunState{
      rs
      | @heater => Device.disable(rs.heater),
  ...
  def enabled?(%RunState{} = rs) do
    heater_enabled?(rs) and air_stir_enabled?(rs) and mist_enabled?(rs) and fresh_air_enabled?(rs)
  end

  def idle_all(%RunState{} = rs) do
    %RunState{
      rs
      | @heater => Device.idle(rs.heater, nil),
